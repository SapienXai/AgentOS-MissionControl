...
test("OpenClaw 2026.9.3 is the recommended fresh baseline while 9.1 remains supported", () => {
  assert.equal(OPENCLAW_RECOMMENDED_VERSION, "2026.9.3");
  assert.equal(OPENCLAW_SUPPORTED_BASELINE_VERSION, "2026.9.1");
  assert.equal(LOCAL_OPENCLAW_COMPATIBILITY_MANIFEST.recommendedVersion, "2026.9.3");
  assert.deepEqual(
    LOCAL_OPENCLAW_COMPATIBILITY_MANIFEST.versions.find((entry) => entry.version === "2026.9.3"),
    {
      version: "2026.9.3",
      status: "certified",
...
test("fresh baseline certification does not invoke the historical migration engine", () => {
  ...
  const evidence = JSON.parse(readFileSync(
    path.join(process.cwd(), "docs/evidence/openclaw-2026.9.3-fresh-baseline.json"),
    "utf8"
  )) as {
    success?: boolean;
